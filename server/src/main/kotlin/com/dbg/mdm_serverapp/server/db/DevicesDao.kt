...
import com.dbg.mdm_serverapp.server.model.Device
import com.dbg.mdm_serverapp.server.model.DeviceInfo
...
class DevicesDao(
    ...
    private val changeBus: DbChangeBus? = null,
) {
    /**
    ...
    fun register(
        ...
        if (existing == null) {
            ...
            val device = find(id)!!
            publish(
                DbChangeEvent.DeviceRegistered(
                    device = device.toDto(),
    ...
    fun markAllOffline() {
        transaction(database) {
            ...
            publish(DbChangeEvent.DevicesMarkedOffline(onlineDeviceCount = 0))
        }
    }
    ...
    private fun publish(event: DbChangeEvent) {
        changeBus?.publish(event)
    }

    private fun Device.toDto(): StatusDeviceDto =
        StatusDeviceDto(
            id = id,
            name = name,
            platform = platform,
